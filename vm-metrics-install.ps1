<#
.SYNOPSIS
    Installs vm-metrics.ps1 as a Windows service.
...
$logPath     = Join-Path $PSScriptRoot "vm-metrics.log"
$errPath     = Join-Path $PSScriptRoot "vm-metrics-error.log"
...
# ── Final status ────────────────────────────────────────────────────────────
if ($running) {
    Write-Host "Metrics available at: http://localhost:9090/metrics" -ForegroundColor Green
} else {
