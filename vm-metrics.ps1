...
function Get-PrometheusMetrics {
    ...
    $lines += "# HELP hyperv_vm_memory_assigned_bytes VM assigned memory in bytes"
    ...
    foreach ($vmName in $vmNames) {
        ...
        $state = if ($vm.State -eq 'Running') { 1 } else { 0 }
        $cpu = $vm.CPUUsage
        $memBytes = $vm.MemoryAssigned
        ...
        $lines += "hyperv_vm_memory_assigned_bytes{vm=`"$vmName`"} $memBytes"
